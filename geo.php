...
if(isset($ip) && $ip && isset($name) && $name && isset($key) && $key) {
  ...
  if($rs->rowCount() > 1)
    exit(showError(500,'Too much nodes with mac='.$mac.', name='.$name));
  ...
  if($rs->rowCount() == 1){
    ...
    if (!$result['readonly']) {
      $updateHood=false;
      if (!$result['isgateway']) {
        // discover the best hood-id from node provided geo-location
...
{
    exit(showError(400,'missing parameter'));
}
